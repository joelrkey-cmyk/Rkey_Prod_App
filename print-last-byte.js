...
try {
  ...
  // Parse ASN.1 to get prime1 and prime2
  let offset = 0;
  function readTLV() {
    if (offset >= buffer.length) return null;
    const type = buffer[offset]; offset++;
    let length = buffer[offset]; offset++;
    if (length & 0x80) {
  ...
  function readInnerTLV() {
    ...
    const type = innerBuffer[innerOffset]; innerOffset++;
    let length = innerBuffer[innerOffset]; innerOffset++;
    if (length & 0x80) {
      const numBytes = length & 0x7f;
      length = 0;
      for (let i = 0; i < numBytes; i++) {
  ...
  console.log('Complete calculated qInv hex:', hex);
  console.log('Length of calculated qInv in bytes:', hex.length / 2);
  console.log('The last byte of calculated qInv is:', hex.substring(hex.length - 2));
} catch (e) {
  console.error(e);
}
